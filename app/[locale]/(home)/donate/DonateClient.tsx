  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden pt-24 pb-16 relative">
      <div className="max-w-2xl mx-auto px-4">
        <Link href="/" className="inline-flex items-center gap-2 text-sm text-white/40 hover:text-white mb-12 transition-colors group font-medium">
            ...
            {methods.map((method) => (
              <div key={method.id} className="border-b border-white/5 last:border-0">
                ...
                >
                  ...
                  <Icon icon="lucide:chevron-down" width="16" className={`text-white/30 shrink-0 transition-transform duration-300 ${selected === method.id ? "rotate-180 text-white/70" : ""}`} />
                </button>

                {selected === method.id && (
                  <div className="bg-[#131315] border-t border-white/5 px-5 py-6 animate-in fade-in slide-in-from-top-2 duration-200">
                    {method.id === "yape" && (
                      <div className="space-y-4 max-w-md">
                        <div className="flex flex-col items-center p-5 bg-black border border-white/10 rounded-lg">
                          <button onClick={() => setQrModalSrc(method.qrImage || null)} className="relative w-32 h-32 bg-white rounded flex items-center justify-center overflow-hidden cursor-pointer group hover:ring-2 hover:ring-white/20 transition-all">
                            <img src={method.qrImage} alt="QR" className="w-full h-full object-contain" />
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors flex items-center justify-center">
                              <Icon icon="lucide:zoom-in" width="24" className="text-black opacity-0 group-hover:opacity-60" />
                            </div>
                          </button>
                        </div>
                    ...
                    {method.id === "paypal" && (
                      <div className="space-y-4 max-w-md">
                        ...
                        <a href={method.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 text-sm font-medium text-white transition-colors">
                          <Icon icon="lucide:external-link" width="16" className="text-white/50" />
                          {t("openLink")}
                        </a>
                      </div>
                    )}
      ...
      {qrModalSrc && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setQrModalSrc(null)}>
          <div className="relative w-full max-w-sm bg-[#131315] border border-white/10 rounded-2xl p-6 shadow-2xl flex flex-col items-center animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="w-full flex justify-between items-center mb-6">
              <span className="text-sm font-semibold text-white/90 tracking-wide">{t("scanQR")}</span>
              <button onClick={() => setQrModalSrc(null)} className="text-white/40 hover:text-white bg-white/5 hover:bg-white/10 rounded-full p-1.5 transition-colors">
                <Icon icon="lucide:x" width="18" />
              </button>
            </div>
            <div className="w-full aspect-square bg-white rounded-xl overflow-hidden flex items-center justify-center p-2">
              <img src={qrModalSrc} alt="QR Ampliado" className="w-full h-full object-contain" />
            </div>
...
function CopyRow({ label, value, copyKey, copied, onCopy, copyText, copiedText }: CopyRowProps) {
  return (
    <div className="flex items-center justify-between bg-black border border-white/10 rounded-lg p-3 group">
      <div>
        <p className="text-[10px] uppercase tracking-wider text-white/40 mb-1 font-medium">{label}</p>
        <p className="font-mono text-sm text-white/90 tracking-wide">{value}</p>
      </div>
      <button 
      ...
      >
        <Icon icon={copied === copyKey ? "lucide:check" : "lucide:copy"} width="14" />
